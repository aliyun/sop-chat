...
def find_reference_doc_path(project_dir: str, output_path: str) -> str | None:
    """Resolve output_path to an existing logstore reference markdown file."""
    if not output_path or not output_path.endswith(".md"):
        return None
    if output_path.endswith("-queries-extra.md"):
        return None
...
def prepare_audit(
    ...
    # Build audit logstores with scores; check reference doc and skip if missing
    audit_logstores = []
    skipped = []
    for rank, name in enumerate(audit_names, 1):
        ...
        reference_doc_path = find_reference_doc_path(project_dir, output_path) if output_path else None
        if not reference_doc_path:
            skipped.append({
                "name": name,
                "reason": "reference doc not found",
                "output_path": output_path or "(none)",
            })
            log(f"Skipping {name}: reference doc not found (output_path={output_path})")
            continue
