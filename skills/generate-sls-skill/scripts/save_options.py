...
Usage:
    python save_options.py <project_dir> [--validate-queries] [--reference-dir <path>] \
...
def main():
    ...
    log(f"[1/3] Scanning logstores in {project_dir}")
    ...
    # [2/3] Build reference mapping
    log("[2/3] Building reference mapping")
    ...
    # [3/3] Write skill_options.json for each logstore
    log(f"[3/3] Writing skill_options.json for {len(logstore_names)} logstores")

    results = []
    for name in logstore_names:
        opts_path = os.path.join(project_dir, name, "skill_options.json")
        existed = os.path.exists(opts_path)
