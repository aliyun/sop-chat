#!/usr/bin/env python3
"""
Unit tests for assemble_overview.py — reference docs and extra query files.
...
class TestReferenceAssembly(unittest.TestCase):
    """Reference docs should mention extra queries only when they exist."""

    def test_without_extra_no_mention(self):
        """When queries_extra.md does not exist, usage instruction should NOT mention an extra file."""
        logstore_dir = _make_logstore_dir()
        output_dir = os.path.join(tempfile.mkdtemp(), "skills", "k8s-log", "references")
        output_path = os.path.join(output_dir, "audit.md")
        ...
        self.assertTrue(content.startswith("# test-logstore"))
        self.assertNotIn("---\nname:", content)
        self.assertIn("## 使用说明", content)
        self.assertNotIn("audit-queries-extra.md", content)
        self.assertIn("本文档中的查询示例", content)

    def test_with_extra_mentions_it(self):
        """When queries_extra.md exists, usage instruction SHOULD mention the alias extra file."""
        logstore_dir = _make_logstore_dir()
        output_dir = os.path.join(tempfile.mkdtemp(), "skills", "k8s-log", "references")
        output_path = os.path.join(output_dir, "audit.md")
        _setup_required_fragments(logstore_dir, output_path)
        ...
        self.assertIn("## 使用说明", content)
        self.assertIn("audit-queries-extra.md", content)
        self.assertIn("优先使用本文档的查询示例", content)
        self.assertIn("找不到时再参考 audit-queries-extra.md", content)

        # Also verify queries_extra.md was copied with the reference doc alias
        extra_output = os.path.join(output_dir, "audit-queries-extra.md")
