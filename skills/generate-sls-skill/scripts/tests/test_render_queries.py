...
from render_queries import (
    render_queries_md, render_selected_md, render_extra_md, render_report,
...
class TestRenderQueriesMd(unittest.TestCase):
    ...
    def test_extra_backlink_uses_reference_doc_name(self):
        """Supplemental query docs link back to the main reference doc when known."""
        entries = [
            {"title": "Extra", "category": "C", "cleaned_query": "* | SELECT 1"},
        ]
        md = render_extra_md(entries, "audit.md")
        self.assertIn("[audit.md](audit.md)", md)
        self.assertNotIn("overview.md", md)
