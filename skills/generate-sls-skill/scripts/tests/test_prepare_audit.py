...
def _create_selected_logstores(
    names: list[str],
    output_path_template: str = "skills/test/references/{name}.md",
) -> dict:
...
def _create_reference_file(project_dir: Path, output_path: str) -> None:
    """Create a stub reference doc so prepare_audit does not skip the logstore."""
    ...
    def test_small_project_full_audit(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for entry in selected["logstores"]:
                _create_reference_file(project_dir, entry["output_path"])
...
class TestReferenceDocSkip(unittest.TestCase):
    """Test that logstores are skipped when reference docs are missing."""

    def test_skipped_when_reference_doc_missing(self):
        """Logstores without reference docs are in skipped list."""
        with tempfile.TemporaryDirectory() as tmp:
            ...
            selected = _create_selected_logstores(names)
            # Do NOT create reference docs, but DO create step_progress.json with completed status
            # to simulate completed logstores that lack output files
            ...
            # Create step_progress.json files with completed status but no reference docs
            for entry in selected["logstores"]:
                logstore_dir = project_dir / entry["name"]
                logstore_dir.mkdir(exist_ok=True)
                (logstore_dir / "step_progress.json").write_text(
            ...
            self.assertEqual(audit_plan["skipped"][0]["reason"], "reference doc not found")
