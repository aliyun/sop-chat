#!/usr/bin/env python3
"""
Assemble a logstore reference document from fragments and LLM-provided metadata.
...
render_queries.py (Step 9), then concatenates them into the final reference doc.
...
Output:
    <output_path>                    (always)
    <output_dir>/<stem>-queries-extra.md
                                     (only when fragments/queries_extra.md exists)
...
def build_reference_header(name: str, description: str) -> str:
    """Build the heading for a bundled logstore reference document."""
    return f"# {name}\n\n> {description}"


def build_usage_instruction(extra_filename: str | None) -> str:
    """Build usage instructions, including the extra-query pointer only when needed."""
    if extra_filename:
        return f"""## 使用说明

- 本 logstore 的查询语法较复杂，建议优先使用本文档的查询示例，找不到时再参考 {extra_filename}，尽量避免自行编写。
...
def insert_extra_pointer(
    ...
    The pointer line format:
        > 本节收录 {N} 条精选查询。更多查询参见 [{extra_filename}]({extra_filename})（{M} 条补充）。
    """
    heading = "## 查询示例"
    pointer = (
        f"> 本节收录 {selected_count} 条精选查询。"
        f"更多查询参见 [{extra_filename}]({extra_filename})（{extra_count} 条补充）。"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Assemble a logstore reference doc from fragments and LLM metadata."
    )
    ...
    extra_filename = f"{output_stem}-queries-extra.md"
    ...
    if os.path.exists(queries_extra_path):
        ...
        queries_selected_md = insert_extra_pointer(
            queries_selected_md, selected_count, extra_count, extra_filename
        )

        # Copy queries_extra.md to output directory with the reference-doc alias.
        os.makedirs(output_dir, exist_ok=True)
        dest_extra = os.path.join(output_dir, extra_filename)
        extra_content = read_file(queries_extra_path) or ""
        with open(dest_extra, "w", encoding="utf-8") as f:
            f.write(extra_content)
        queries_extra_copied = True
        copied_extra_filename = extra_filename
        log(f"Copied queries_extra.md to {dest_extra}")
    else:
        stale_extra = os.path.join(output_dir, extra_filename)
        if os.path.exists(stale_extra):
            os.remove(stale_extra)
            log(f"Removed stale {stale_extra} (extra is empty)")

    # --- Read optional fragments ---
    common_values_md = read_file(os.path.join(fragments_dir, "common_values.md"))

    # --- Assemble reference doc ---
    usage_instruction = build_usage_instruction(copied_extra_filename)
    sections = [
        build_reference_header(args.name, args.description),
        usage_instruction,
