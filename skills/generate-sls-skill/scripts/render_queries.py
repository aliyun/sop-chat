...
Input files (read from <input_dir>):
    ...
    - skill_options.json (optional, for extra-query back link)
...
def render_extra_md(entries: list[dict], main_doc_name: str | None = None) -> str:
    """Render queries_extra.md."""
    count = len(entries)
    backlink = (
        f"精选查询参见 [{main_doc_name}]({main_doc_name})。"
        if main_doc_name else
        "精选查询参见主文档。"
    )
    header = (
        f"# 补充查询\n\n"
        f"> 本文件包含 {count} 条补充查询。{backlink}"
    )
...
def main():
    ...
    main_doc_name = None
    if isinstance(skill_options, dict) and skill_options.get("output_path"):
        main_doc_name = os.path.basename(skill_options["output_path"])
    ...
    if extra:
        extra_md = render_extra_md(extra, main_doc_name)
