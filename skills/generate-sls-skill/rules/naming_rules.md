...
```text
{根目录}/{project目录名}/SKILL.md
{根目录}/{project目录名}/references/{logstore目录名}.md
{根目录}/{project目录名}/references/{logstore目录名}-queries-extra.md   # 仅 extra 查询存在时生成
```
...
**根目录**：
1. 扫描当前工作目录下的所有一级子目录
2. 优先选择包含 SKILL 特征文件的目录（如 `*/SKILL.md`），常见名称 `skills`
3. 找不到时默认使用 `skills/`
4. 在批量确认时展示，用户可修改
...
**Logstore 目录名**：
1. 扫描 `{根目录}/{project目录名}/references/` 下已有 `.md` 文件（排除 `*-queries-extra.md`）
2. 用简化后的 logstore 名匹配已有文件名（不含 `.md`）
3. 匹配到 → 使用已有目录名；无匹配 → 使用简化名
...
| logstore | `audit` | `references/my-audit.md` | `my-audit` |
...
**SKILL 入口**: `{根目录}/{project目录名}/SKILL.md`
...
|--------|---------|------|--------|----------|
| logstore-abc123 | 138 [R] | dashboard:112 alert:17 | logstore | skills/.../references/logstore.md |
```
...
python3 scripts/save_selections.py <project_dir> <<'SELECTIONS'
{
  ...
  "selections": {
    "{logstore_name}": "{根目录}/{project目录名}/references/{logstore目录名}.md",
    ...
  }
}
SELECTIONS
