---
name: generate-sls-skill
description: 将 SLS project 或输入目录转换为结构化的 SKILL 知识文档。当用户提供 SLS project 名称、要求生成日志分析技能文档、从 index/dashboard 生成知识文档、或提到 generate-sls-skill 时触发。
...
支持两种输入方式：
1. **SLS Project 名称**：自动通过 `aliyun` CLI 获取 index/dashboard 数据，然后生成 SKILL 知识文档
2. **本地目录**（输入目录）：直接从已有数据目录生成 SKILL 知识文档
...
- [ ] Step 9：清理 + 标注
- [ ] Step 10：渲染片段 + 报告
- [ ] Step 11：组装 logstore reference 文档
- [ ] `update_status.py <project_dir> --mark-completed <logstore>`

**Phase C**（project skill 入口更新）：
- [ ] Step 12：生成/更新 project `SKILL.md`
...
对 project 和所有 logstore 执行名称简化、去重，并确认输出路径。Read [naming_rules.md](rules/naming_rules.md)，严格按规则执行。

输入：`<project_dir>/project_summary.json`（Step 2 输出）
输出：`<project_dir>/selected_logstores.json` + 各确认 logstore 的 `skill_options.json` 追加 `output_path`（`<output_root>/<project_alias>/references/<logstore_alias>.md`）
...
> 组装结果写入 `skill_options.json` 中 `output_path` 指定的 reference 文档路径；若存在 extra 查询，同时写入同目录 `<logstore_alias>-queries-extra.md`。
...
### Step 12: 生成/更新 project SKILL 入口

> 全局一次性执行（Phase C）。

根据所有 reference 文档生成或更新 `<output_root>/<project_alias>/SKILL.md`。Read [index_rules.md](rules/index_rules.md)，严格按规则执行。

输入：`<project_dir>/selected_logstores.json`（含所有已确认 logstore 的 output_path 和 output_root）
输出：`<output_root>/<project_alias>/SKILL.md`

**Phase C 完成**：提示用户 SKILL 知识文档生成已完成，询问是否需要进行质量审计（Phase D，可选，建议新会话执行）。
