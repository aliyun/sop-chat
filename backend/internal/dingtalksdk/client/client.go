...
import (
	...
	"sop-chat/internal/dingtalksdk/handler"
	"sop-chat/internal/dingtalksdk/logger"
	"sop-chat/internal/dingtalksdk/payload"
	"sop-chat/internal/dingtalksdk/plugin"
	"sop-chat/internal/dingtalksdk/utils"
)
...
func (cli *StreamClient) processLoop() {
	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				logger.GetLogger().Errorf("read goroutine panic: %v", err)
			}
		}()
		for {
